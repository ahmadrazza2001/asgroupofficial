...
import {
  EditAchievers,
  UploadAchieversImage,
} from "../../../apicalls/achiever";
...
function Images({ selectedAchievers, setShowAchieversForm, getData }) {
  ...
  const upload = async () => {
    try {
      ...
      formData.append("achieversId", selectedAchievers._id);
      const response = await UploadAchieversImage(formData);
  ...
  const deleteImage = async (image) => {
    try {
      ...
      const response = await EditAchievers(
        selectedAchievers._id,
        updatedAchievers
      );
      if (response.success) {
        message.success(response.message);
